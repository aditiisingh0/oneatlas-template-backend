...
__export(index_exports, {
  AppSpecSchema: () => AppSpecSchema,
  DeploymentEngine: () => DeploymentEngine,
  EntityDefSchema: () => EntityDefSchema,
  FieldDefSchema: () => FieldDefSchema,
  FieldTypeSchema: () => FieldTypeSchema,
  PageDefSchema: () => PageDefSchema,
  PreviewEngine: () => PreviewEngine,
  RBACPolicySchema: () => RBACPolicySchema,
  REGISTRY: () => REGISTRY,
  RedisPreviewStore: () => RedisPreviewStore,
  RouteDefSchema: () => RouteDefSchema,
  WorkflowExecutor: () => WorkflowExecutor,
  buildSpecFromEntities: () => buildSpecFromEntities,
  composeBundle: () => composeBundle,
  computeSlotValues: () => computeSlotValues,
  deploymentEngine: () => deploymentEngine,
  deserializeManifest: () => deserializeManifest,
  diffFileSets: () => diffFileSets,
  diffSpecs: () => diffSpecs,
  evaluateCondition: () => evaluateCondition,
  generateAppShellFiles: () => generateAppShellFiles,
  generateAppSpec: () => generateAppSpec,
  generateComponentRegistryModule: () => generateComponentRegistryModule,
  generateDynamicDetailPageShell: () => generateDynamicDetailPageShell,
  generateDynamicFormPageShell: () => generateDynamicFormPageShell,
  generateDynamicListPageShell: () => generateDynamicListPageShell,
  generateFileFromTemplate: () => generateFileFromTemplate,
  generateFullPrismaSchema: () => generateFullPrismaSchema,
  generateHydrationManifest: () => generateHydrationManifest,
  generateHydrationModule: () => generateHydrationModule,
  ...
  previewEngine: () => previewEngine,
  refineAppSpec: () => refineAppSpec,
  registerTemplate: () => registerTemplate,
  renderPreviewPage: () => renderPreviewPage,
  resolveTemplate: () => resolveTemplate,
  runDeployPipeline: () => runDeployPipeline,
  runGenerationPipeline: () => runGenerationPipeline,
  runIncrementalUpdate: () => runIncrementalUpdate,
  runPreviewPipeline: () => runPreviewPipeline,
  serializeHydrationManifest: () => serializeHydrationManifest,
...
var FieldDefSchema = import_zod.z.object({
  name: import_zod.z.string().min(1).regex(/^[a-zA-Z][a-zA-Z0-9_]*$/, "Must be valid identifier"),
  ...
  enumValues: import_zod.z.array(import_zod.z.string()).optional(),
  relation: import_zod.z.object({
    targetEntity: import_zod.z.string(),
    type: import_zod.z.enum(["one-to-one", "one-to-many", "many-to-many"]),
    foreignKey: import_zod.z.string().optional()
  }).optional(),
...
var EntityDefSchema = import_zod.z.object({
  name: import_zod.z.string().min(1).regex(/^[A-Z][a-zA-Z0-9]*$/, "Must be PascalCase"),
  pluralName: import_zod.z.string().min(1),
  tableName: import_zod.z.string().min(1).regex(/^[a-z][a-z0-9_]*$/, "Must be snake_case"),
  fields: import_zod.z.array(FieldDefSchema).min(1),
  softDelete: import_zod.z.boolean().optional(),
  timestamps: import_zod.z.boolean().optional(),
  tenantScoped: import_zod.z.boolean().optional()
});
var PageDefSchema = import_zod.z.object({
  id: import_zod.z.string().uuid(),
  path: import_zod.z.string().startsWith("/"),
  component: import_zod.z.string().min(1),
  pageType: import_zod.z.enum(["list", "detail", "form", "dashboard", "custom"]),
  entityName: import_zod.z.string().optional(),
  title: import_zod.z.string().min(1),
  icon: import_zod.z.string().optional(),
  permissions: import_zod.z.array(import_zod.z.string()).optional(),
  slots: import_zod.z.record(import_zod.z.unknown()).optional()
});
var RouteDefSchema = import_zod.z.object({
  method: import_zod.z.enum(["GET", "POST", "PUT", "PATCH", "DELETE"]),
  path: import_zod.z.string().startsWith("/"),
  handler: import_zod.z.string().min(1),
  entityName: import_zod.z.string().optional(),
  middlewares: import_zod.z.array(import_zod.z.string()).optional(),
  rateLimit: import_zod.z.object({
    requests: import_zod.z.number().positive(),
    windowMs: import_zod.z.number().positive()
  }).optional()
});
var RBACPolicySchema = import_zod.z.object({
  roles: import_zod.z.array(import_zod.z.object({
    name: import_zod.z.string().min(1),
    description: import_zod.z.string().optional(),
    inherits: import_zod.z.array(import_zod.z.string()).optional()
  })),
  entityPermissions: import_zod.z.array(import_zod.z.object({
    entityName: import_zod.z.string().min(1),
    role: import_zod.z.string().min(1),
    actions: import_zod.z.array(import_zod.z.enum(["create", "read", "update", "delete", "list"]))
  }))
});
var AppSpecSchema = import_zod.z.object({
  id: import_zod.z.string().uuid(),
  tenantId: import_zod.z.string().min(1),
  appType: import_zod.z.enum(["crud", "dashboard", "workflow", "admin-panel"]),
  name: import_zod.z.string().min(1).max(100),
  slug: import_zod.z.string().regex(/^[a-z0-9-]+$/, "Must be URL-safe slug"),
  description: import_zod.z.string().optional(),
  entities: import_zod.z.array(EntityDefSchema).min(1),
  pages: import_zod.z.array(PageDefSchema).min(1),
  routes: import_zod.z.array(RouteDefSchema).min(1),
  permissions: RBACPolicySchema,
  workflows: import_zod.z.array(import_zod.z.any()).optional(),
  integrations: import_zod.z.array(import_zod.z.any()).optional(),
  ...
  meta: import_zod.z.object({
    ...
    generatedBy: import_zod.z.enum(["ai", "user", "template"]),
...
function composeBundle(spec, manifest) {
  ...
  const bundleHash = (0, import_crypto2.createHash)("sha256").update(files.map((f) => f.content).join("")).digest("hex").slice(0, 16);
  return {
    appId: spec.id,
    tenantId: spec.tenantId,
    files,
